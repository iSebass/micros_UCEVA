...
void PCF_Wr(uint8_t data){
    if(statusLED==1) data |= (1<<3); else data &= ~(1<<3);
...
void _write_nibble(uint8_t nibble){
    if( (nibble & 0x01) == 0) word &= ~(1<<4); else word |= (1<<4);
    if( (nibble & 0x02) == 0) word &= ~(1<<5); else word |= (1<<5);
    if( (nibble & 0x04) == 0) word &= ~(1<<6); else word |= (1<<6);
    if( (nibble & 0x08) == 0) word &= ~(1<<7); else word |= (1<<7);

    word &= ~(1<<2); PCF_Wr(word);
    word |= (1<<2); PCF_Wr(word);
    word &= ~(1<<2); PCF_Wr(word);
...
void _send_byte(uint8_t value, uint8_t mode){
    ...
    switch(mode){
        case 0: word &= ~(1<<0); break;
        case 1: word |= (1<<0); break;
    }
...
void i2c_lcd_init(){
    ...
    i2c_lcd_command( 0x20 | 0x00 | 0x08 | 0x00 );
    _delay((unsigned long)((37)*(64000000UL/4000000.0)));

    _lcd_params = 0x04 | 0x02 | 0x01;
    i2c_lcd_command(0x08 | _lcd_params);
    _delay((unsigned long)((37)*(64000000UL/4000000.0)));
...
void i2c_lcd_scroll_left(void){
  i2c_lcd_command(0x10 | 0x08 | 0x00);
  _delay((unsigned long)((37)*(64000000UL/4000000.0)));
}
void i2c_lcd_scroll_right(void){
  i2c_lcd_command(0x10 | 0x08 | 0x04);
  _delay((unsigned long)((37)*(64000000UL/4000000.0)));
}

void i2c_lcd_custom_char(uint8_t mem, uint8_t *charmap){
 i2c_lcd_command(0x40 | ((mem&0x07)<<3) );
 for(uint8_t i=0; i<=7; i++){
  i2c_lcd_write(charmap[i]);
 }
 i2c_lcd_command(0x80);
 _delay((unsigned long)((37)*(64000000UL/4000000.0)));
}
void i2c_lcd_out(char row, char col, char *str){
